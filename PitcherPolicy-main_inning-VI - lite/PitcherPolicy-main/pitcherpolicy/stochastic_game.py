...
from state import Count,Inning
...
class StochasticGame:
    ...
    -------
    solve_game()
        runs value iteration and solve_lp to solve the game
    ...
    def __init__(self, states: List[Inning], trans_prob_mat: dict) -> None:
    ...
    def run_val_iter(self, theta: float = 0.001):
        ...
        while True:
            ...
            for state in self.states:
                ...
                for pitch in self.trans_prob_mat[state.atbat]:
                    ...
                    for zone in self.trans_prob_mat[state.atbat][pitch]:
                        ...
                        for res, res_prob in self.trans_prob_mat[state.atbat][pitch][zone][count][
                            ...
                            '''
                            if res in [Outcomes.SINGLE.value,Outcomes.DOUBLE.value,Outcomes.TRIPLE.value,Outcomes.HOMERUN.value]:
                                q_vals[state.state_name][pitch][zone][
                                    BatActs.SWING.value
                                ] += res_prob

                            elif res not in [Outcomes.GROUNDOUT.value]:
    ...
    def print_solution(self, state_vals: List[dict], state_policy: dict) -> None:
        ...
        """
        for state in self.states:
            #print(state_vals[state.state_name])
            print(
